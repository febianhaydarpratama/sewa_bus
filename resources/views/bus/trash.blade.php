...
<a href="{{ route('bus.index') }}" type="button" class="btn btn rounded-3">Data Pesanan</a>
...
<h4 class="mt-5">Data Trash Bus</h4>
<div class="form-search" style="float:right">
    <form action="{{ route('bus.search_trash') }}" method="get" accept-charset="utf-8">
        <div class="form-group" style="display:flex">
            <input type="text" id="nama" name="nama" class="form-control" placeholder="Kode bus">
...
<table class="table table-hover mt-2">
    ...
    <tbody>
        ...
        <tr>
                ...
                <td>{{ $data->harga }}</td>
            <td>
                <a href="{{ route('bus.restore', $data->id_bus) }}" type="button"
                    class="btn btn-success rounded-3">Pulihkan</a>
                <!-- TAMBAHKAN KODE DELETE DIBAWAH BARIS INI -->
                <!-- Button trigger modal -->
                <button type="button" class="btn btn-dark" data-bs-toggle="modal"
                    data-bs-target="#hapusModal{{ $data->id_bus }}">
                    Hapus
                </button>
                <!-- Modal -->
                <div class="modal fade" id="hapusModal{{ $data->id_bus }}" tabindex="-1"
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form method="POST" action="{{ route('bus.delete', $data->id_bus) }}">
                                @csrf
